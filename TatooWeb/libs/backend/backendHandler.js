...
var backendHandler = function () {
    ...
    var apiUrl = {
        ...
        openFeedback: baseUrl + '/feedbackpanel/feedback/',
    };
    ...
    var dateRangePickerHandler = function () {
    ...
    var editArtwork = function () {
        ...
        $.post(apiUrl.editArtwork, { art: art }, function (data) {
            setArtUpdated(data.Id, data.ShowOnIntroScreen, art.ArtDesc);
            ...
            message += '<br/><br/><span>' + art.ArtDesc + '</span>';
        }).fail(function (jqXHR) {
    ...
    var openFeedback = function (id) {
        $.get(apiUrl.openFeedback + id, function (response) {
            $('#feedback h4.modal-title').text('Feedback from ' + response.CustomerName);
            $('#icon-' + id).attr('src', '/Content/images/site_images/read-email.png');
            $('#fb-mail').text(!!response.CustomerEmail ? response.CustomerEmail : '...');
            $('#fb-phone').text(!!response.CustomerPhone ? response.CustomerPhone : '...');
            $('#fb-message').text(response.Message);
        }).fail(function () {
            $('#feedback h4.modal-title').text('Unable to load feedback');
            $('#fb-mail').text('...');
            $('#fb-phone').text('...');
            $('#fb-message').text('...');
        });
